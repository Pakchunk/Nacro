#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: PlayerCameraMode_DBNO

#include "Basic.hpp"

#include "V3_PlayerCameraModeBase_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass PlayerCameraMode_DBNO.PlayerCameraMode_DBNO_C
// 0x0000 (0x11821D80 - 0x11821D80)
class UPlayerCameraMode_DBNO_C final : public UV3_PlayerCameraModeBase_C
{
public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"PlayerCameraMode_DBNO_C">();
	}
	static class UPlayerCameraMode_DBNO_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UPlayerCameraMode_DBNO_C>();
	}
};
static_assert(alignof(UPlayerCameraMode_DBNO_C) == 0x11821D80, "Wrong alignment on UPlayerCameraMode_DBNO_C");
static_assert(sizeof(UPlayerCameraMode_DBNO_C) == 0x11821D80, "Wrong size on UPlayerCameraMode_DBNO_C");

}

