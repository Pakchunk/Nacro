#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: V2_PlayerCameraModeRanged

#include "Basic.hpp"

#include "V2_PlayerCameraModeBase_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass v2_PlayerCameraModeRanged.v2_PlayerCameraModeRanged_C
// 0x0000 (0x11821D80 - 0x11821D80)
class UV2_PlayerCameraModeRanged_C : public UV2_PlayerCameraModeBase_C
{
public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"v2_PlayerCameraModeRanged_C">();
	}
	static class UV2_PlayerCameraModeRanged_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UV2_PlayerCameraModeRanged_C>();
	}
};
static_assert(alignof(UV2_PlayerCameraModeRanged_C) == 0x11821D80, "Wrong alignment on UV2_PlayerCameraModeRanged_C");
static_assert(sizeof(UV2_PlayerCameraModeRanged_C) == 0x11821D80, "Wrong size on UV2_PlayerCameraModeRanged_C");

}

